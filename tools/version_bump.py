...
def write_version(version):
    ...
    for n, v in zip(version_names, version_values):
        version_line = "{}_VERSION = ".format((n))
        content = re.sub("{}.*\n".format((version_line)), "{}{}\n".format((version_line), (v)), content)
...
def main():
    ...
    subprocess.run(["git", "commit", "-nam", "Release {}".format((bumped))])

    if arguments.tag:
        subprocess.run(["git", "tag", str(bumped), "-m", "Release {}".format((bumped))])
