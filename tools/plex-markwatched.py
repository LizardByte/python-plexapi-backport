...
if __name__ == '__main__':
    ...
    print('{} Starting plex-markwatched script..'.format((datestr())))
    plex = PlexServer()
    for section in plex.library.sections():
        print('{} Checking {} for unwatched items..'.format((datestr()), (section.title)))
        for item in _iter_items(section.search(collection='markwatched')):
            if not item.isWatched:
                print('{}  Marking {} watched.'.format((datestr()), (_get_title(item))))
                item.markWatched()
    # Check all OnDeck items
    print('{} Checking OnDeck for unwatched items.'.format((datestr())))
