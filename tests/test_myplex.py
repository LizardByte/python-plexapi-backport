...
def test_myplex_accounts(account, plex):
    ...
    print("home: {}".format((account.home)))
    ...
    print("signInState: {}".format((account.signInState)))
    assert account.username, "Account has no username"
...
def test_myplex_resources(account):
    ...
    for resource in resources:
        ...
        print("{} ({}): {}".format((name), (resource.product), (connections)))
    assert resources, "No resources found for account: {}".format((account.username))
...
def test_myplex_devices(account):
    ...
    for device in devices:
        ...
        print("{} ({}): {}".format((name), (device.product), (connections)))
    assert devices, "No devices found for account: {}".format((account.name))
...
def test_myplex_users(account):
    ...
    assert user, "Could not find user {}".format((users[0].title))

    try:
        users[0].servers[0]
    except IndexError:
        return pytest.skip("{} shared user does not have access to any servers".format((users[0].title)))
