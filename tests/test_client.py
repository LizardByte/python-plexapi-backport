...
def _check_capabilities(client, capabilities):
    ...
    for capability in capabilities:
        if capability not in supported:
            pytest.skip(
                "Client {} doesn't support {} capability support {}".format((client.title), (capability), (supported))
            )
...
def test_client_playback(plex, client, movies, proxy):
    ...
    try:
        ...
        print("client.playMedia({})".format((movie.title)))
        client.playMedia(movie)
        time.sleep(5)
        print("client.pause({})".format((mtype)))
        client.pause(mtype)
        time.sleep(2)
        print("client.stepForward({})".format((mtype)))
        client.stepForward(mtype)
        time.sleep(5)
        print("client.play({})".format((mtype)))
        client.play(mtype)
        time.sleep(3)
        print("client.stepBack({})".format((mtype)))
        ...
        time.sleep(10)
        print("client.stop({})".format((mtype)))
        client.stop(mtype)
        time.sleep(1)
    finally:
