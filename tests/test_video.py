...
def test_video_ne(movies):
    assert (
        len(
            movies.fetchItems(
                "/library/sections/{}/all".format((movies.key)), title__ne="Sintel"
            )
        )
        == 3
    )
...
def test_video_Movie_getStreamURL(movie, account):
    ...
    assert url.startswith("{}/video/:/transcode/universal/start.m3u8".format((utils.SERVER_BASEURL)))
    assert account.authenticationToken in url
    assert "path={}".format((quote_plus(key))) in url
    assert "protocol" not in url
    assert "videoResolution" not in url

    url = movie.getStreamURL(videoResolution="800x600", protocol='dash')
    assert url.startswith("{}/video/:/transcode/universal/start.mpd".format((utils.SERVER_BASEURL)))
