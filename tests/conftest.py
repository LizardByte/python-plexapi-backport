...
SYNC_DEVICE_IDENTIFIER = "test-sync-client-{}".format((plexapi.X_PLEX_IDENTIFIER))
SYNC_DEVICE_HEADERS = {
...
def shared_username(account):
    ...
    for user in account.users():
        if user.title.lower() == username.lower():
            ...
            return username
    pytest.skip("Shared user {} wasn't found in your MyPlex account".format((username)))
...
def wait_until(condition_function, delay=0.25, timeout=1, *args, **kwargs):
    ...
    assert ready, "Wait timeout after {} retries, {:.2f} seconds".format((int(retries)), (time.time() - start))
