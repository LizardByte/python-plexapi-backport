...
def test_Playlist_create(plex, show):
    ...
    try:
        ...
        items = playlist.reload().items()
        assert items[3].ratingKey == episodes[3].ratingKey, 'Missing added item: {}'.format((episodes[3]))
        # Test add two items
        playlist.addItems(episodes[4:6])
        items = playlist.reload().items()
        assert items[4].ratingKey == episodes[4].ratingKey, 'Missing added item: {}'.format((episodes[4]))
        assert items[5].ratingKey == episodes[5].ratingKey, 'Missing added item: {}'.format((episodes[5]))
        assert len(items) == 6, 'Playlist should have 6 items, {} found'.format((len(items)))
        # Test remove item
        toremove = items[5]
        playlist.removeItems(toremove)
        items = playlist.reload().items()
        assert toremove not in items, 'Removed item still in playlist: {}'.format((items[5]))
        assert len(items) == 5, 'Playlist should have 5 items, {} found'.format((len(items)))
        # Test remove two item
        toremove = items[3:5]
        playlist.removeItems(toremove)
        items = playlist.reload().items()
        assert toremove[0] not in items, 'Removed item still in playlist: {}'.format((items[3]))
        assert toremove[1] not in items, 'Removed item still in playlist: {}'.format((items[4]))
        assert len(items) == 3, 'Playlist should have 5 items, {} found'.format((len(items)))
    finally:
        playlist.delete()
